...
/*PAESE BLUETOOTH STRING*/ 
    ...
    if(recieved_string != "")
    {
       ...
       if(recieved_string == "HORN_ON")
       {
           digitalWrite(horn_pin, HIGH);//turn off the headlight to start
           digitalWrite(horn_pin, HIGH);//turn off the headlight to start
       }
       if(recieved_string == "HORN_OFF")
       {
           digitalWrite(horn_pin, LOW);//turn off the headlight to start
           digitalWrite(horn_pin, LOW);//turn off the headlight to start
       }
    }
